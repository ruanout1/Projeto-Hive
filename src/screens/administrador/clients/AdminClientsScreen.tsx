...
import { DeleteClientDialog } from './components/ClientDeleteDialog';
...
export default function AdminClientsScreen({ onBack }: AdminClientsScreenProps) {
  const { clients, saveClient, deleteClient, toggleClientStatus } = useClients();
    ...
    <div className="h-full bg-gray-50">
      <div className="bg-white border-b border-gray-200 p-6">
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center justify-between mb-6">
            ...
            <Button onClick={handleCreate} style={{ backgroundColor: '#6400A4', color: 'white' }}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div className="bg-purple-50 rounded-xl p-4 border-2 border-[#6400A4] flex justify-between items-center">
               <div><p className="text-sm text-gray-600">Total</p><p className="text-2xl text-[#6400A4] font-bold">{clients.length}</p></div>
            ...
            <div className="bg-green-50 rounded-xl p-4 border-2 border-green-500 flex justify-between items-center">
               <div><p className="text-sm text-gray-600">Ativos</p><p className="text-2xl text-green-600 font-bold">{activeCount}</p></div>
            ...
            <div className="bg-red-50 rounded-xl p-4 border-2 border-red-500 flex justify-between items-center">
               <div><p className="text-sm text-gray-600">Inativos</p><p className="text-2xl text-red-600 font-bold">{inactiveCount}</p></div>
            ...
            <div className="bg-yellow-50 rounded-xl p-4 border-2 border-yellow-400 flex justify-between items-center">
               <div><p className="text-sm text-gray-600">Receita</p><p className="text-2xl text-gray-800 font-bold">R$ {totalRevenue.toLocaleString('pt-BR', { minimumFractionDigits: 0, maximumFractionDigits: 0 })}</p></div>
      ...
      <div className="max-w-7xl mx-auto p-6">
        <ClientList 
          ...
          onView={handleView}
          onEdit={handleEdit}
          onDelete={handleDeleteClick} // DELETE ATIVADO
      ...
      {viewingClient && (
        <ClientViewDialog isOpen={isViewOpen} onClose={() => setIsViewOpen(false)} client={viewingClient} onEdit={(client) => { setIsViewOpen(false); setEditingClient(client); setIsFormOpen(true); }} />
      )}
